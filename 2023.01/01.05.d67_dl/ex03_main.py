...
from ex02_custom_dataset import CustomDataset
import pandas as pd
...
def main():    
    ...
    train_dataset= CustomDataset("./0105/dataset/train", transform = train_aug)
    valid_dataset= CustomDataset("./0105/dataset/valid", transform = valid_aug)
    test_dataset = CustomDataset("./0105/dataset/test" , transform = valid_aug)
    ...
    model1.head = nn.Linear(in_features=768, out_features=450)
    model1.to(device)

    model2 = models.__dict__["resnet50"](pretrained= True)
    model2.fc = nn.Linear(in_features= 2048, out_features= 450)
    model2.to(device)

    model3 = models.__dict__["resnet18"](pretrained= True)
    model3.fc = nn.Linear(in_features= 512, out_features= 450)
    model3.to(device)

    model4 = models.__dict__["vgg16"](pretrained= True)
    model4.classifier[6] = nn.Linear(in_features= 4096, out_features= 450)
    model4.to(device)

    model5 = models.__dict__["shufflenet_v2_x0_5"](pretrained= True)
    model5.fc = nn.Linear(in_features= 4096, out_features= 450)
    model5.to(device)

    # model_list= [model1, model2, model3]
    model_list= [model4, model5]
    ...
    for index, model in enumerate(model_list):
        ...
        save_path = f'./0105/best_4.pt'
        dfForAccuracy = pd.DataFrame(index=list(range(epochs)),
                                    columns=['Epoch', 'Accuracy'])

        if os.path.exists(save_path) :
            best_val_acc = max(pd.read_csv(f'./0105/modelAccuracy_{str(index)}.csv')['Accuracy'].tolist())
        ...
        for epoch in range(epochs) :
            ...
            if epoch == epochs - 1 :
                dfForAccuracy.to_csv(f"./0105/modelAccuracy_{str(index)}.csv" , index=False)
