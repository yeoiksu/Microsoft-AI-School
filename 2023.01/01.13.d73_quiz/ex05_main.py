...
def main():    
    ...
    train_dataset = CustomDataset("./0113/dataset/train" , transform= train_aug)
    valid_dataset = CustomDataset("./0113/dataset/val"   , transform= valid_aug)
    ...
    for index, model in enumerate(model_list):
        ...
        save_path = f'./0113/best_{model_try}.pt'
        dfForAccuracy = pd.DataFrame(index=list(range(epochs)),
                                    columns=['Epoch', 'Accuracy', 'Loss'])

        if os.path.exists(save_path) :
            best_val_acc = max(pd.read_csv(f'./0113/{model_names}_{model_try}.csv')['Accuracy'].tolist())
        ...
        for epoch in range(epochs) :
            ...
            if epoch == epochs - 1 :
                dfForAccuracy.to_csv(f"./0113/{model_names}_{model_try}.csv" , index=False)
