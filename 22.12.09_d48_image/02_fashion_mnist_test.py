...
file_path = './22.12.09_d48_image/data'
...
kwargs = {'num_workers': 1, 'pin_memory': True} if use_cuda else{}  
                # 1: 사용 프로세서 1로설정
print('>> Set vars and deivce done')

batch_size = 64     # 한번 학습할때 몇개
test_batch_size = 1000

dataset_train = CustomImageDataset(
    annotations_file= file_path + '/FashionMNIST/annotation_train.csv',
    img_dir= file_path + '/FashionMNIST/train',
    )

dataset_test = CustomImageDataset(
    annotations_file= file_path + '/FashionMNIST/annotation_test.csv',
    img_dir= file_path + '/FashionMNIST/test',
    )
...
def train(log_interval, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)  # 64(batch size), 1, 28, 28(input 28 x 28) 
        optimizer.zero_grad()
        output = model(data)
