...
def image_file_check(opt):
    ...
    new_path = image_path.replace(f"images", "dataset/images")
    ...
    # banana
        if index == 0:
            print("바나나 데이터 양\t:", image_len)            
    # kiwi
        elif index == 1:
            print("키위 데이터 양\t\t:", image_len)            
    # orange
        elif index == 2:
            print("오렌지 데이터 양\t:", image_len)            
    # pineapple
        elif index == 3:
            print("파인애플 데이터 양\t:", image_len)            
    # watermelon
        elif index == 4:
            print("수박 데이터 양\t\t:", image_len)            
...
def image_resize(file_path):
    data = glob.glob(os.path.join(file_path, "*.jpg"))
