...
class Ui_MainWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        for proc in psutil.process_iter() :
            if proc.name() == "python.exe":
                print('프로그램 시작된 지금 모든 프로세스는 >>>', proc)
    ...
    def live_webcam_click_on(self):
        ...
        if self.webcam_flag:
            opt = self.parse_opt()
            self.tread1 = Process(target = self.run(**vars(opt)), name='WebcamClickOn')
            self.tread1.start()
            self.tread1.join()   
            # ProcessorCheck()
        print(self.webcam_flag)
    ...
    def run(
            ...
            send_DB = True, # 데이터 베이스로 데이터를 보낼지 말지 
        ...
        for frame_idx, batch in enumerate(dataset):
            ...
            for i, det in enumerate(p):  # detections per image
                ...
                if (send_DB) and (len(individual_List) > 0):
                    DB_start = 5     
                    now_DB = datetime.now() 
