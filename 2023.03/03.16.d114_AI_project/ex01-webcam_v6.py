import cv2, sys, numpy as np, torch, argparse, os, psutil
...
'''ui폼 받아오기'''
# Main
form = resource_path('ex01_main.ui')
...
def ProcessorCheck():
    pid, name = multiprocessing.current_process().pid, multiprocessing.current_process().name
...
class Ui_MainWindow(QMainWindow, form_class):
    def __init__(self):
        ...
        # self.th1 = ''        
        self.frame = QLabel(self.frame_webcam)
        self.tread1 = None # 아무 상태가 아닌 인스턴스로 한 번 선언해줘야 한다고 합니다

        ''' 웹캠 동작(종료)시키는 클릭 이벤트 처리 연결'''
        self.pushButton_cctv_on.clicked.connect(partial(self.live_webcam_click_on))  # 동작
        self.pushButton_cctv_off.clicked.connect(partial(self.live_webcam_click_off)) # 종료 
    ...
    def live_webcam_click_on(self):
        ''' 실제 카메라 화면 출력 부분'''
        # ProcessorCheck()
        self.opt.web_flag = True
        if self.opt.web_flag:
            # self.opt.web_flag = False
            self.tread1 = Process(target = self.run(**vars(self.opt)), name='WebcamClickOn')

            self.tread1.start()
            self.tread1.join()   
            
            print("="*50)
            print("BUTTON ON 2 ", self.webcam_flag)
            print("="*50)
    ...
    def live_webcam_click_off(self):
        ...
        self.frame.clear() 
        self.frame.hide() # 정확히는 숨김 처리 --> 노트   # 1
        # ProcessorCheck()
        # self.tread1.terminate()                          # 2
        print("="*50)
        print(f"BUTTON OFF && WEBCAM FLAG : {self.opt.web_flag}")
        print("="*50)

        deleteCam(self.webcam_flag)
    ...
    def run(
            ...
            reid_weights=WEIGHTS / 'best_m_model.pt',  # model.pt path,
                ...
                for i, det in enumerate(p):  # detections per image                    
                    ...
                    if det is not None and len(det):
                        ...
                        for numb_track, name_track, tracked_time in individual_List:
                            ...
                            if numb_track in target_List :
                                ...
                                print('\n------numb_track------- : ',numb_track, "FUCCCKKKK", web_flag)
                            #elif frame_List
                    ...
                    if send_DB:
                        ...
                        if int(now_DB.second) % send_DB_term == DB_start and is_data_sent == False:    
                            ...
                            th2 = Process(target = DB_process(frame_List), name='SendDB')
                            th2.start()
                    ...
                    if web_flag == True:
                        if send_EMAIL and is_email_first == True: 
                            ...
                            if len(target_List)>=1:
                                ...
                                th3 = Process(target = send_alarm(
                                    target_object,is_email_first,
                                    len(target_List), t_before, 
                        ...
                        if send_EMAIL and is_email_first == False: 
                            ...
                            if len(target_List) >= 1:
                                ...
                                if t_cal >= send_EMAIL_term:
                                    ...
                                    th4 = Process(target = send_alarm(
                                        target_object,is_email_first,len(target_List), 
                                        t_before, current_time, address),
                                        name='SendEmailNotFirst')
                                    th4.start()
    ...
    def parse_opt(self):
            ...
            parser.add_argument('--reid-weights', type=Path, default=WEIGHTS / 'best_m_model.pt')
            ...
            parser.add_argument('--send_EMAIL', action='store_true', default=False, help='T /F send date to EMAIL')
            ...
            parser.add_argument('--web_flag', action='store_true', default=False, help='T/F webcam on/off')
